...
#include <unistd.h>
	...
	public:
		...
		bool Open(unsigned short port)
		{
			...
			/*if (*/
			bind(socket, (const sockaddr*)&address, sizeof(sockaddr_in)); /* < 0)
			{
	...
	protected:
		...
		void UpdateQueues()
		{
			...
			if (receivedQueue.size())
			{
				...
				const unsigned int minimum_sequence = latest_sequence >= 34
					? (latest_sequence - 34)
	...
	private:
		...
		// received packets for determining acks to send (kept up to most recent recv sequence - 32)
	...
	public:
		...
		{
			const int header = 12;
			unsigned char* packet = new unsigned char[(const int)header + size];
	...
	private:
		...
		// reliability system: manages sequence numbers and acks, tracks network stats etc.
	};
}
